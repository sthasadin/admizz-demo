  ...
  if (college) {
    ...
    college?.college_rankings?.map((rank, index) => {
      ...
      if (index == 0) rankTitles.push(object);
        ...
        if (count == 0) rankTitles.push(object);
      }
    });
    ...
    rankTitles.map((rank) => {
      ...
      college?.college_rankings?.map((data) => {
        if (rank.title.toLowerCase() == data.rank_id.title.toLowerCase()) {
          if (Number(data.year) == firstYear) rank.ranks[0] = data.rank;
          else if (Number(data.year) == secondYear) rank.ranks[1] = data.rank;
          else if (Number(data.year) == thirdYear) rank.ranks[2] = data.rank;
          else if (Number(data.year) == fourthYear) rank.ranks[3] = data.rank;
          count++;
        }
      });
      if (count == college.college_rankings.length) return rank;
    });
